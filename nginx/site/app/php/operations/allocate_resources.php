<?php
    ...
    if (!userHasWebPermission($accessor['user_tag'], 8) && $accessor['user_tag'] != $target_user['user_tag'])
    {
    ...
    // In case the user is trying to change the allocated resources of the admin account, check if they are the admin
    if ($target_user['user_tag'] == 'admin')
    {
        http_response_code(200);
        echo json_encode(array("error" => "You cannot change the allocated resources of the admin account.", "element-name" => "{$modal_id}-error"));
        exit();
    }
    
    // Actually changes the allocated resources
    $manager->update('User', 'max_ram', $new_role, "user_tag = '{$target_user['user_tag']}'");
    $manager->getConnector()->close();
    
    http_response_code(200);
    echo json_encode(array("success" => "Successfully changed allocated resources.", "method" => "POST",
